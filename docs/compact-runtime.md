...
- 정교한 감정/추세/심각도, AI 재판단, 자동 실행 권고, KPI 값 추정은 기본 경로에서 생략.
- 공식 자료는 항상 분석 후보로 유지한다. 공개 반응은 운영·장애·결제·확률·밸런스·보상·공정성 등
  사업 관련성이 확인된 자료만 AI 입력에 포함하며, 개인 신변·잡담·모집·팬아트·개인 빌드 공유는 제외한다.
- Slack은 게임당 대표 항목 1개, Notion은 게임당 최대 2개를 표시한다. 원본 수집 기록과
  검증된 전체 결정 데이터는 보존하므로 플랫폼 표시 제한이 수집 누락을 의미하지 않는다.
- 요약 검증 실패는 원문을 기록하지 않고 항목 수, 한국어, 근거 ID, 출처 역할, taxonomy,
  측정 주장 등 안전한 원인 코드로 구분한다.
- 서로 독립된 요약 항목 중 일부만 검증에 실패하면 통과 항목은 유지하고 안전한 경고 코드만 남긴다.
  모든 항목이 실패한 게임만 분석 공백으로 처리한다.
- 관련성 선별 규칙 도입 시 분석 상태 버전을 분리해 기존 결론을 새 규칙의 결과처럼 재사용하지 않는다.
  검증된 사건 category와 BM taxonomy에 직접 연결되는 경우에만 게임당 최대 4개의
  `pm_metric_context` 및 내부 확인 질문을 결정론적으로 생성한다. 값·증감·인과관계는 추정하지 않는다.
  공식 내용 P2, 반응만 있으면 P3: 긴급 탐지 서비스가 아니다.
