...
1. 수정된 YouTube 경로를 GitHub에서 유료 분석 없이 한 번 검증한다.
2. 로컬에 남은 승인된 보고서 디자인 변경을 검토·업로드한다.
3. Slack 전용 앱과 Notion 대상 페이지를 준비하고 권한·설정 존재 여부를 확인한다.
4. 별도 발송 승인 후 소량 보고서로 Notion 생성 → Slack 링크 수신 → 중복 방지를 통합 검증한다.
5. 성공 확인 후 실발송 스위치를 활성화하고 첫 예약 실행 결과를 확인한다.
