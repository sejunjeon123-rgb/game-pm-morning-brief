...
현재는 **수집·요약·검증·보고서 포맷·전송 코드와 Slack/Notion 연결 검증이 완료된 상태**다. 2026-09-08 사용자 승인으로 `live_delivery_enabled=true`가 적용되어 예약 실행은 실제 수집·분석 후 Notion과 Slack에 전송한다.
...
현재 조립 규칙은 공식 사실이 있으면 P2, 주장 중심이면 P3이며 `VERIFY`, `LOW`를 사용한다. P0/P1 긴급 탐지, 정교한 감정·추세 분석, 내부 KPI 값 추정, 운영 조치 자동 권고는 기본 경로에서 제외했다. 검증된 사건 category와 BM taxonomy에 직접 연결되는 경우에만 최대 4개의 `pm_metric_context`와 내부 확인 질문을 결정론적으로 추가하며, 공개 자료만으로 값·증감·인과관계를 판단하지 않는다.
...
| 공개 반응 선별 | 운영·장애·결제·확률·밸런스·보상·공정성 관련 자료만 분석 후보로 사용 |
| 플랫폼 표시 | Slack 게임당 1개, Notion 게임당 최대 2개 |
