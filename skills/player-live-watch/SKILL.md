...
For deterministic active-source collection, use `player_live_watch.common_collector.collect_player_live_evidence` or `python -m app.run --mode player-live-collect`. The common collector currently combines official YouTube RSS as `OFFICIAL_FACT` and DCInside posts as `PLAYER_CLAIM`; it must preserve those classifications through downstream analysis. The DCInside adapter scans bounded listing pages, excludes fixed notices, surveys, and ads, never reads or stores writer fields, and fetches a bounded mix of newest and source-engaged post bodies. It must prefetch one listing for every scoped game before detail reads and pace live requests to prevent one high-volume gallery from starving later games. When a verified listing is available but a selected detail body remains inaccessible after bounded retries, preserve a `TITLE_ONLY` claim and record the detail coverage gap instead of discarding the item. Treat view, recommendation, and comment counts only as source-specific selection metadata. A bounded scan that does not reach the full seven-day window must return a coverage gap rather than imply complete coverage.
